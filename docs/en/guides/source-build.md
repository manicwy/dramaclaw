...
| Custom init times out / `Request timed out .../newapi/init` | Two common causes: (1) the UI sends `127.0.0.1:3000`, which is not the gateway inside the api container — use `http://newapi:3000`; (2) a host `HTTP_PROXY` (Clash, etc.) intercepts `newapi` and returns 502, while `/setup` in the browser still works. Rebuild api and retry. Compose adds `newapi` to `NO_PROXY`. |
